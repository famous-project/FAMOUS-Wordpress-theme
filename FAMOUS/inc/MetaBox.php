...
add_action('admin_head', array('FAMOUS_MetaBox', '_global_head'));

add_action('admin_footer', array('FAMOUS_MetaBox', '_global_foot'));

define('FAMOUS_MODE_ARRAY', 'array');

define('FAMOUS_MODE_EXTRACT', 'extract');

define('FAMOUS_FIELD_HINT_TEXT', 'text');

define('FAMOUS_FIELD_HINT_TEXTAREA', 'textarea');

define('FAMOUS_FIELD_HINT_CHECKBOX', 'checkbox');

define('FAMOUS_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');

define('FAMOUS_FIELD_HINT_RADIO', 'radio');

define('FAMOUS_FIELD_HINT_SELECT', 'select');

define('FAMOUS_FIELD_HINT_SELECT_MULTI', 'select_multi');

// depreciated, use FAMOUS_FIELD_HINT_SELECT_MULTI instead
define('FAMOUS_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

define('FAMOUS_LOCK_TOP', 'top');

define('FAMOUS_LOCK_BOTTOM', 'bottom');

define('FAMOUS_LOCK_BEFORE_POST_TITLE', 'before_post_title');

define('FAMOUS_LOCK_AFTER_POST_TITLE', 'after_post_title');

define('FAMOUS_VIEW_START_OPENED', 'opened');

define('FAMOUS_VIEW_START_CLOSED', 'closed');

define('FAMOUS_VIEW_ALWAYS_OPENED', 'always_opened');

class FAMOUS_MetaBox
{
	/**
	 ...
	 * FAMOUS_MODE_ARRAY (default) and FAMOUS_MODE_EXTRACT, this option
	 * should be used when instantiating the class.
	 *
	 * @since	1.2
	 * @access	public
	 * @var		string
	 */
	var $mode = FAMOUS_MODE_ARRAY;

	/**
	 * When the mode option is set to FAMOUS_MODE_EXTRACT, you have to take
	...
	function FAMOUS_MetaBox($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = FAMOUS_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = FAMOUS_LOCK_BOTTOM;
	...
	{
		if (FAMOUS_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! FAMOUS_MetaBox::_is_post() AND ! FAMOUS_MetaBox::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (FAMOUS_LOCK_TOP == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'FAMOUS-side-top';
				if ( ! $('#'+id).length)
				{
					$('<div></div>').attr('id',id).prependTo('#side-info-column');
				}
				<?php else: ?>
				var id = 'FAMOUS-content-top';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (FAMOUS_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'FAMOUS-side-bottom';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (FAMOUS_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'FAMOUS-content-bpt';
				if ( ! $('#'+id).length)
				{
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (FAMOUS_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'FAMOUS-content-apt';
				if ( ! $('#'+id).length)
				{
				...
				<?php if (FAMOUS_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (FAMOUS_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (FAMOUS_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		$prefix = 'FAMOUS_filter_' . $this->id . '_';
	...
	{
		$prefix = 'FAMOUS_action_' . $this->id . '_';
		$prefix = preg_replace('/_+/', '_', $prefix);
	...
	function _is_post()
	{
		if ('post' == FAMOUS_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	function _is_page()
	{
		if ('page' == FAMOUS_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = FAMOUS_MetaBox::_get_current_post_type();
	...
	function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = FAMOUS_MetaBox::_get_post_id();
		...
		jQuery(function($)
		{
			...
			{		
				...
				if (elem.attr('class') && elem.filter('[class*=dodelete]').length)
				{
					...
					if (confirm('Wirklich löschen? Beachte das die Daten nicht wiederherstellbar sind!'))
					{
						if (the_name)
						{
			...
			$('[class*=docopy-]').click(function(e)
			{
				...
				$.FAMOUS.trigger('wpa_copy', [the_clone]);
			});
	...
	function _global_foot()
	{
		...
		(function($){ /* not using jQuery ondomready, code runs right away in footer */
			...
			$.FAMOUS = $('<div></div>').attr('id','FAMOUS').appendTo('body');
	...
	{
		if (!$this->in_template AND $this->mode == FAMOUS_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			FAMOUS_FIELD_HINT_CHECKBOX_MULTI,
			FAMOUS_FIELD_HINT_SELECT_MULTI,
			FAMOUS_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	function _save($post_id) 
	{
		...
		if ($this->mode == FAMOUS_MODE_EXTRACT)
		{
			$new_fields = array();
			...
			// keep data tidy, delete values if previously using FAMOUS_MODE_ARRAY
		...
		else
		{
			...
			// keep data tidy, delete values if previously using FAMOUS_MODE_EXTRACT
