 ...
 class FAMOUS_Box
{
	...
	public function __construct(array $a = array())
	{
		...
		if ( ! defined('FAMOUS_SEND_TO_EDITOR_ENABLED'))
		{
			add_action('admin_footer', array($this, 'init'));

			define('FAMOUS_SEND_TO_EDITOR_ENABLED', true);
		}
	}
		...
		if ($uri AND in_array($file, array('post.php', 'post-new.php')))
		{
			...
			/* <![CDATA[ */
				...
				jQuery(function($)
				{
					...
					{
						var FAMOUS_insert_button_label = '';

						var FAMOUS_mediafield = null;

						var FAMOUS_send_to_editor_default = send_to_editor;

						send_to_editor = function(html)
						{
							clearInterval(interval);

							if (FAMOUS_mediafield)
							{
								...
								FAMOUS_mediafield.val(url);

								// reset insert button label
								setInsertButtonLabel(FAMOUS_insert_button_label);

								FAMOUS_mediafield = null;
							}
							else
							{
								FAMOUS_send_to_editor_default(html);
							}
						...
						$('[class*="<?php echo $this->button_class_name; ?>"]').on('click', function()
						{
							...
							FAMOUS_mediafield = $('.<?php echo $this->field_class_name; ?>-' + name, $(this).closest('.postbox'));
							...
							function iframeSetup()
							{
								...
								{
									// run once
									if ( ! FAMOUS_insert_button_label.length)
									{
										FAMOUS_insert_button_label = getInsertButtonLabel();
									}
